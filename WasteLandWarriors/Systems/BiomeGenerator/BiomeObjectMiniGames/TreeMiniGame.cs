...
{
    public class TreeMiniGame : IBiomeObjectMiniGame
    {
        public Player p;
        ...
        void OnClickHammer(object sender, ClickPlayerTextDrawEventArgs e)
        {
            ...
            if (e.PlayerTextDraw == Hammer)
            {
                ...
                {
                    progress = 0;
                    p.inventory.slotsItem.FirstOrDefault(l => l.loot.Name == "Топор").condition -= 5;
                    //p.ApplyAnimation("BASEBALL", "BAT_4", 10.0f,false,false,false,false,2000);
                    p.SendClientMessage("Вы добыли дерево.");
                    p.inventory.AddItem(Loot.loots.FirstOrDefault(l => l.Name == "Дрова").Id);
